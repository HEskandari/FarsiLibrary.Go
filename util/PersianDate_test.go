...
func TestPersianDate_Format(t *testing.T) {
	pd, _ := NewPersianDate(1388, 2, 3)
	if assert.NotNil(t, pd) {
		assert.Equal(t, "۱۳۸۸/۰۲/۰۳", pd.Format(GenericFormat))
		assert.Equal(t, "اردیبهشت ۱۳۸۸", pd.Format(MonthYearFormat))
		assert.Equal(t, "۳ اردیبهشت", pd.Format(MonthDayFormat))
		assert.Equal(t, "۱۳۸۸/۲/۳", pd.Format(GenericShortFormat))
		assert.Equal(t, "پنجشنبه ۳ اردیبهشت ۱۳۸۸", pd.Format(WrittenFormat))
...
func TestPersianDate_GetDayOfWeek(t *testing.T) {
	...
	pd, _ := NewPersianDate(1387, 7, 7) //7 Mehr equals Yekshanbe
	wd := pd.DayOfWeek()
